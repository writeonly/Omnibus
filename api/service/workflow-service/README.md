...
`workflow-service` orchestrates rule publication requests. It receives publication submissions, starts the publication workflow, and validates candidate rules through `rule-service` before publication.
...
3. workflow-service starts the publication workflow.
4. workflow-service calls rule-service to validate the full ruleset.
5. rule-service compiles the candidate Drools rules.
