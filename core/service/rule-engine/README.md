...
Drools-based bridge bidding rule engine.

## Responsibility

`rule-engine` parses bridge hands, builds bidding facts, evaluates bundled and managed Drools rules, and returns bidding recommendations. It also exposes rule administration paths and can publish domain events for audit/archive consumers.
...
4. API gateway routes to rule-engine.
